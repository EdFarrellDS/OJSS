...
class Account:
    ...
    def update_skills(self):
        ...
        if not globAccountsDF.skills[globAccountsDF.UID == globUID].tolist() == ['[]']:
            self.current_skills = ast.literal_eval(globAccountsDF.skills[globAccountsDF.UID == globUID][0])
...
class JobRecruiter:
    ...
    def get_jobs():
        return globJobsDF[globJobsDF.jobCreatorUID == globUID]
...
class Menu:
    ...
    def admin_menu():
        ...
        request = input("Please select the number option you wish to proceed with.")
        if request == '1':
            SystemAdministratorController().create_job_category()
        elif request == '2':
            SystemController.logout()
        else:
            print("\nInput not recognised.")
